            ...
            <form className={''} onSubmit={handleSubmit(onFormSubmit)}>
                <div className="px-0 gap-3 flex lg:items-end items-center ">
                    <div className='pt-4 gap-3 flex flex-col lg:flex-row items-center basis-3/4'>
                        ...
                        <InputTextField size={'slx'}
                                        name="gender"
                                        type="text"
                                        value={passenger.gender || ''}
                                        onChange={(e)=>handleChange(e.target.name, e.target.value)}>جنسیت</InputTextField>
                    </div>
                    <div className="flex flex-col rounded-lg gap-2 basis-1/4">
                        ...
                        <DatePicker
                                ...
                                onChange={(date) => {
                                    ...
                                    setBirthDate(`${_date.getFullYear()}-${_date.getMonth() + 1}-${_date.getDate()}`)
                                    handleChange('birth_day',`${_date.getFullYear()}-${_date.getMonth() + 1}-${_date.getDate()}`)
                                }}
                        />
                    </div>
                </div>
                <div className="my-7 pt-4 px-0 w-full gap-3 flex items-end ">
                    <div className='gap-3 flex flex-col lg:flex-row items-center basis-3/4'>
                        <InputTextField size={'slx'}
                                        ...
                                        onChange={(e)=>handleChange(e.target.name, e.target.value)}>ملیت</InputTextField>
                        <InputTextField size={'slx'}
                                        name="national_code"
                                        type="text"
                                        value={passenger.national_code || ''}
                                        onChange={(e)=>handleChange(e.target.name, e.target.value)}>کد ملی</InputTextField>
                        <InputTextField size={'slx'}
                                        name="passport_number"
                                        type="text"
                                        value={passenger.passport_number || ''}
                                        onChange={(e)=>handleChange(e.target.name, e.target.value)}>شماره پاسپورت</InputTextField>
                    </div>
                    <div className=" rounded-lg gap-2 basis-1/4  flex flex-col justify-end flex-wrap lg:fle lg:items-start">
                        ...
                        <DatePicker
                            ...
                            onChange={(date) => {
                                const _date = new Date(date.toDate())
                                setExpirePassport(`${_date.getFullYear()}-${_date.getMonth() + 1}-${_date.getDate()}`)
                                handleChange('expire_date_passport',`${_date.getFullYear()}-${_date.getMonth() + 1}-${_date.getDate()}`)
                            }}
                        />
                    </div>
                </div>
